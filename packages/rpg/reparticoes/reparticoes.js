...
class ReparticaoInit {
    ...
    maisProximo(Position) {
        let prevdist = 100000.000;
        let reparticaoID;
        this.marker.forEach((value, index) => {
            const dist = this.marker[index].dist(Position);
            if (dist < prevdist) {
                prevdist = dist;
                reparticaoID = index;
            }
...
exports.mercadinho = new ReparticaoInit([
