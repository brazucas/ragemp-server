...
mp.events.addCommand("savepos", (player, name = "Sem descrição") => {
    ...
    fs_1.default.appendFile(saveFile, `[${player.name}] Posição: ${pos.x}, ${pos.y}, ${pos.z} [Região: ${mp.world.zone.getName3d(pos)}] | ${(player.vehicle) ? `Rotação: ${rot.x}, ${rot.y}, ${rot.z}` : `Ângulo: ${rot}`} | ${(player.vehicle) ? "Em veículo" : "A pé"} - ${name}\r\n`, (err) => {
        if (err) {
...
mp.events.add("servidor:getGroundZFor3dCoord", (player, _groundZ) => {
    groundZ = _groundZ;
});
