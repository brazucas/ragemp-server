...
const reparticoes_1 = require("../reparticoes/reparticoes");
mp.events.addCommand("mercadinho", (player) => {
    const id = reparticoes_1.mercadinho.maisProximo(player.position);
    if (id != undefined) {
        reparticoes_1.mercadinho.blip[id].routeFor(player, 2, 1);
...
mp.events.addCommand("trabalhar", (player) => {
    ...
    const noMarcador = reparticoes_1.mercadinho.noMarcador(player);
    if (!player.status.trabalhando && noMarcador === true) {
        player.status.trabalhando = true;
        player.PizzaBoy = new PizzaBoyInit(player);
    }
    else
        player.notify('Você não está em um mercadinho ou já está trabalhando!');
});
mp.events.addCommand("entregar", (player) => {
    ...
    if (player.PizzaBoy.indoEntrega) {
        ...
        player.notify('Você não está entregando!');
});
class PizzaBoyInit {
    constructor(player) {
        this.posEntrega = [
    ...
    gerarEntrega(player) {
        let randPos = this.posEntrega[Math.floor(Math.random() * this.posEntrega.length)];
